	...
	while (job != NULL)
	{
		...
		**********************************************************************/
		if (required_memory <= memory) {
			...
			// if the job does not have enough time to complete before the time quantum is reached for round robin, execute job and add it to the end of the jobs list.
			if(job->required_time > time_quantum && mode == RR) {
				execute_job(job);
				enqueue(jobs, job);
			}
			
			// if mode is not round robin or job has enough time to complete before the time quantum is reached, execute job.
			else {				
				execute_job(job);
			}
		}
	...
	d_linked_list_t *list)
{
	...
	**************************************************************************/
	...
	{
		pthread_mutex_lock(&mutexLock1); // lock to synchronize threads.
...
void execute_job(job_t *job) {
	...
	******************************************************************/
	...
	pthread_mutex_unlock(&mutexLock1); // unlock mutual exclusion once jobs are done being executed.
	...
	******************************************************************/ 
	...
	if (mode == RR) {
		sleep(time_quantum); // if mode is round robin, sleep for time quantum.
	}
	
	else {
		sleep(job->required_time); // otherwise, sleep for job's required time. 
	}
	/******************************************************************
	...
	******************************************************************/
	...
	if (mode == RR) {
		job->required_time -= time_quantum; // if mode is round robin, change job's required time/
	}	
}
